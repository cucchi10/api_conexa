...
import { portBusiness } from './config/config';
...
// Main errorHandler
app.use((error: any, req: express.Request, res: express.Response, next: express.NextFunction) => { errorHandler(error, req, res, next); });
...
app.listen(portBusiness, () => {
  console.log(`App listening at ${portBusiness}`);
});